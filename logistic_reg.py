...
class LogisticRegression():
    ...
    def predict(self, X):
        y_predict = self.sigmoid(np.dot(X, self.weights) + self.bias)
        y_predict_labels = y_predict > 0.5 #return True or False

        return y_predict_labels

    def sigmoid(self, z):
        return 1/ (1 + np.exp(-z)) #sigmoid/ logistic function

if __name__ == '__main__':
    np.random.seed(1)
    X, y = make_blobs(n_samples=1000, centers=2) #2 classes
    y = y[:, np.newaxis] #0 and 1. two classes
    # print(y)
...
#     Thinking: realizing a multiclass classification
#     Hint: Using one vs all rules
